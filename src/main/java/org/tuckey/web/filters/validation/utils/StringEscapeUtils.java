...
// <!-- rsaquo is proposed but not yet ISO standardized -->
    ...
    private static void addEntities(String[][] entityArray) {
        for (int i = 0; i < entityArray.length; ++i) {
            map.put(Integer.valueOf(entityArray[i][1]), entityArray[i][0]);
        }
    }
    ...
    public static String escapeHtml(String str) {
        ...
        for (i = 0; i < str.length(); ++i) {
            ...
            String entityName = (String) map.get(new Integer(ch));
            if (entityName == null) {
                if (ch > 0x7F) {
                    buf.append("&#");
                    buf.append(ch);
